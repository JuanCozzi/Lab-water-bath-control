...
uint16_t tiempo[]={0,0,0,0,0,0};
uint8_t  temperatura[] ={0,0,0,0,0,0,0};
...
int main(void){
  ...
  lcd_char(3);
    ...
    lcd_string(msg_out);
		sleep_ms(1000);
		gpio_put(GPIO_LED_RP,GPIO_OFF);
    if( gpio_get(GPIO_SET) == GPIO_OFF) lcd_init();
    lcd_set_cursor(0,0);
    lcd_string(msg[2]);
		sleep_ms(1000);
